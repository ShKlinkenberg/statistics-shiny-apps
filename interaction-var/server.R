...
shinyServer(function(input, output) {
  ...
  output$formulamult <- renderUI({
    ...
    withMathJax(
      helpText(
        paste0("$$\\small{\\color{black}{attitude = ",
              ...
              " * smoker + }\\color{",
              unname(brewercolors["Green"]),
              "}{",
              round(as.numeric(input$coefficientslider),2),
              "}\\color{black}{ * exposure * smoker}}$$ \n $$\\small{\\color{",
              unname(brewercolors["Blue"]),
              "}{Non-smoker}\\color{black}{: attitude = ",
              round(as.numeric(results[[1]]),2), 
              " + ",
              round(as.numeric(results[[2]]),2),
              " * exposure + }\\color{DarkGray}{",
              round(as.numeric(results[[3]]),2),
              " * }\\color{",
              unname(brewercolors["Blue"]),
              "}{0}\\color{black}{ + }\\color{",
              ...
              "}{",
              round(as.numeric(input$coefficientslider),2),
              "}\\color{DarkGray}{ * exposure * }\\color{",
              unname(brewercolors["Blue"]),
              "}{0}}$$ \n $$\\small{\\color{",
              unname(brewercolors["Red"]),
              "}{Smoker}\\color{black}{: attitude = ",
              round(as.numeric(results[[1]]),2), 
              " + ",
              round(as.numeric(results[[2]]),2),
              " * exposure + ",
