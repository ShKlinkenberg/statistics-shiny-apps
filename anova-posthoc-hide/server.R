    ...
    ggplot(df,aes(x = x, y = y)) +
      ...
      geom_segment(x = end - 2.55,
                   ...
                   arrow = arrow(length = unit(2,"mm"),
                                 ends = "both",
                                 type = "closed"),
                   alpha = ifelse(input$selectBox, 1, 0)) + 
      ...
      geom_text(label = paste0("'eta'^2 == ", format(round(etasqrd,2), nsmall = 2)),
                ...
                parse = TRUE) +
      #Fvalue text label
      geom_text(label = format(paste0("F (",
                               format(df1And2[1]),
                               ", ",
                               format(df1And2[2]),
                               ") = ",
      ...
      geom_label(label = pprint(phJolCloon),
                x = ifelse(input$selectBox, end - 2.5, -5),
      ...
      x = ifelse(input$selectBox, end - 1.5, -5),
      ...
      x = ifelse(input$selectBox, end - 0.18, -5),
      y = mean(c(mean(subset(df, cat == "Clooney")$y),
                 mean(subset(df, cat == "NoEnd")$y))
      ),
      hjust = 0,
